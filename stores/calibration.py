#!/usr/bin/env python3
# ── SAFE App Store vendor note (The Forge, calibration ledger, 2026-08-11) ────
# Vendored byte-for-byte (except this header block) from the oakenscrolls-office
# playground build in THIS repo: `apps/oakenscrolls-office/calibration.py`
# (b17: OAKOF), Apache-2.0 — same license as this store, zero friction.
#
# Why vendor rather than import the playground copy: `apps/` is the contested
# commons (rule 5) — a store-side organ (D1) must not import a build out of the
# untrusted playground, and `apps/oakenscrolls-office/` is not on the store's
# import path anyway. This is pure stdlib (`math`), ~40 lines, small enough to
# audit in one sitting — the same "vendor the core, not the package" discipline
# `friction_floor`/`human_loop` already follow. Rule 11: the house already grew
# this (oakenscrolls-office's confidence mirror, the box's calibration tool);
# the Forge re-points it at the MODEL's own confidence instead of rebuilding the
# Brier/log-score/reliability math. Kept diffable against the playground copy on
# purpose — if oakenscrolls' copy moves, `diff` this against it and reconcile.
#
# A "pair" everywhere below is (confidence, outcome): the final P(true) the
# predictor stated, and whether the claim turned out true.
"""
