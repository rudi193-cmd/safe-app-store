...
from mcp_connect.registry import McpServerSpec, get_server, get_server_by_name, load_server_env

log = logging.getLogger("mcp_connect.client")
...
def connect_server(server_id: str, timeout: float = 45) -> dict[str, Any]:
    ...
    if spec is None:
        spec = get_server_by_name(server_id)
    if spec is None:
    ...
    _sessions[spec.server_id] = holder
    return {
        "ok": True,
