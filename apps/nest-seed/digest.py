...
def build_digest(db_path: str, model: str = _embed.DEFAULT_EMBED_MODEL) -> str:
    ...
    cats = q("""select label,count(*) n from fragments where label!='' and fragment_type in
    ...
    L.append(f"_A map of `{os.path.dirname(db_path) or '.'}` — {src_total} sources, "
             f"{frag_total:,} fragments. Generated locally; nothing left this machine._\n")
