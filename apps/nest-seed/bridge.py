...
def build_bridge(db_path: str, model: str = _embed.DEFAULT_EMBED_MODEL) -> dict:
    ...
    cats = q(f"""select label, count(*) n from fragments
    ...
    atoms.append({
        ...
        "summary": (
            ...
            f"categories by size: {cat_line}. {len(disc)} auto-discovered category "
            f"clusters. Source: nest-seed bridge (structure only, no content)."
        ),
        "category": "nest",
