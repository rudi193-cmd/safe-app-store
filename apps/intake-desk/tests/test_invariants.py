...
def test_tampered_body_is_detectable(vault, tmp_path):
    conn, _, sid = vault
    assert desk_db.verify_bodies(conn) == []
    # Bypass the trigger the way a restored backup or a raw sqlite3 session
    # would, and prove the digest still catches it.
    conn.execute("DROP TRIGGER statements_body_write_once")
...
def test_claims_are_never_deleted(vault):
    conn, _, sid = vault
    cid = desk.add_claim(conn, statement_id=sid, span=(0, 40), assertion="The shop closed.")
    with pytest.raises(sqlite3.IntegrityError, match="never deleted"):
...
def test_span_beyond_the_body_is_refused(vault):
    ...
    with pytest.raises(desk.DeskError, match="does not resolve"):
        desk.add_claim(conn, statement_id=sid, span=(0, len(BODY) + 1), assertion="x")


def test_inverted_span_is_refused(vault):
    conn, _, sid = vault
    with pytest.raises(desk.DeskError, match="does not resolve"):
        desk.add_claim(conn, statement_id=sid, span=(30, 10), assertion="x")


def test_quoted_returns_the_source_words(vault):
    conn, _, sid = vault
    cid = desk.add_claim(conn, statement_id=sid, span=(0, 15), assertion="The shop closed.")
...
def test_uncheckable_is_a_successful_outcome(vault):
    conn, _, sid = vault
    cid = desk.add_claim(
        conn, statement_id=sid, span=(41, len(BODY)),
...
def test_queue_puts_contradictions_where_a_human_is_needed(vault):
    conn, _, sid = vault
    a = desk.add_claim(conn, statement_id=sid, span=(0, 20), assertion="a")
    b = desk.add_claim(conn, statement_id=sid, span=(20, 40), assertion="b")
    ...
    assert b  # the un-docketed claim is the uncorroborated one
